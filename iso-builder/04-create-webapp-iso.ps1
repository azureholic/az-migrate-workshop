# Create bootable Ubuntu Webapp Autoinstall ISO using xorriso
# Reads config from autoinstall-config-webapp/ and includes webapp/ directory
...
$outputIso = Join-Path $outputDir "ubuntu-24.04.3-webapp-autoinstall.iso"
$autoinstallDir = Join-Path $PSScriptRoot "auto-install-configs\autoinstall-config-webapp"
...
Write-Host "=== Ubuntu Webapp Autoinstall ISO Builder ===" -ForegroundColor Cyan
Write-Host "Includes: Node.js + Express + PostgreSQL + PM2" -ForegroundColor Cyan
Write-Host ""
...
# Ensure Unix line endings on all text files that go into the ISO
Write-Host "Ensuring Unix line endings..."
...
foreach ($pattern in @('*.sql', '*.sh', '*.service')) {
    foreach ($file in (Get-ChildItem -Path $webappDir -Filter $pattern -ErrorAction SilentlyContinue)) {
        $content = Get-Content -Raw $file.FullName
        [System.IO.File]::WriteAllText($file.FullName, $content.Replace("`r`n", "`n"), [System.Text.UTF8Encoding]::new($false))
    }
}
...
$wslWebappDir = ConvertTo-WslPath $webappDir
...
if ($LASTEXITCODE -ne 0) {
    ...
    # Copy webapp directory into ISO
    $isoWebappDir = Join-Path $extractPath "webapp"
    New-Item -ItemType Directory -Force -Path $isoWebappDir | Out-Null
    Copy-Item -Path "$webappDir\*" -Destination $isoWebappDir -Recurse -Force
...
if ($LASTEXITCODE -eq 0 -and (Test-Path $outputIso)) {
    ...
    Write-Host "Webapp ISO created successfully!" -ForegroundColor Green
    Write-Host "========================================" -ForegroundColor Green
    ...
    Write-Host "  Webapp:    http://<vm-ip>:3000"
    Write-Host "  PostgreSQL: webadmin / webadmin123"
    Write-Host "  SSH:        ubuntu / ubuntu"
} else {
